const pinchDistance = 0.02;
      ...
      AFRAME.registerComponent('manos', {
         ...
         updateSkeleton: function () {
            ...
            // Obtener el cubo (el objeto con el que queremos detectar la colisión)
            // const cube = document.querySelector('#cube');  // Asegúrate de que el cubo tenga el ID correcto

            // Asegurarte de que el cubo tenga un collider
            // if (!cube.hasAttribute('dynamic-body')) {
            //     cube.setAttribute('dynamic-body', 'shape: box; mass: 1'); // Agregar un collider al cubo
            // }
                ...
                if (inputSource.handedness === this.data.hand && inputSource.hand) {
                    for (const [jointName, jointEntity] of Object.entries(this.joints)) {
                        ...
                        if (jointPose) {
                            ...
                            // Asegurarte de que cada articulación tenga un collider
                            // if (!jointEntity.hasAttribute('dynamic-body')) {
                            //     jointEntity.setAttribute('dynamic-body', 'shape: sphere; mass: 0.1'); // Agregar un collider a la articulación
                            // }
                            // if (!jointEntity.hasAttribute('obb-collider')) {
                            //     jointEntity.setAttribute('obb-collider', 'target: #cube;'); // Colisionador con el cubo
                            // }
                        } else {
         ...
         detectGesture: function () {
            ...
            // Escuchar el evento 'collide' y emitir 'collide-start'
            // this.el.sceneEl.addEventListener('collide', (evt) => {
            //    if (evt.detail.body.el.id === (this.data.hand === 'left' ? 'left-hand' : 'right-hand')) {
            //       this.el.emit('collide-start', { hand: this.data.hand });
            //    }
            // });
         }
      });
      ...
      AFRAME.registerComponent('detector', {
         ...
         init: function () {
            ...
            // this.el.sceneEl.addEventListener('collide-start', (evt) => {
            //    if (evt.detail.hand === this.data.hand) {
            //       this.updateText(`¡Colisión iniciada con la mano ${this.data.hand}!`);
            //    }
            // });
         },
